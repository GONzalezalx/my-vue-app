...
export default {
  ...
  setup(props) {
    ...
    try {
      const saved = localStorage.getItem('uniformeBSP.tableData');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed)) {
          tableData.value = parsed.map(r => ({ nombre: r.nombre || '', nombreb: r.nombreb || '', tallasa: r.tallasa || '', tallasu: r.tallasu || '' }));
        }
      }
    } catch (e) { void 0; }
    ...
    watch(tableData, (val) => {
      try { localStorage.setItem('uniformeBSP.tableData', JSON.stringify(val)); } catch (e) { void 0; }
    }, { deep: true });

    function makeDefaultRow() {
      return { nombre: '', nombreb: '', tallasa: '', tallasu: '' };
    }
    ...
    function addRow() {
      ...
      try { localStorage.setItem('uniformeBSP.tableData', JSON.stringify(tableData.value)); } catch (e) { void 0; }
