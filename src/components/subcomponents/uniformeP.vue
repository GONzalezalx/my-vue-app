...
export default {
  ...
  setup(props) {
    ...
    try {
      const saved = localStorage.getItem('uniformeP.tableData');
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed)) {
          tableData.value = parsed.map(r => ({ nombre: r.nombre || '', nombrec: r.nombrec || '', tallac: r.tallac || '' }));
        }
      }
    } catch (e) { void 0; }
    ...
    watch(tableData, (val) => {
      try { localStorage.setItem('uniformeP.tableData', JSON.stringify(val)); } catch (e) { void 0; }
    }, { deep: true });

    function makeDefaultRow() {
      return { nombre: '', nombrec: '', tallac: '' };
    }
    ...
    function addRow() {
      ...
      try { localStorage.setItem('uniformeP.tableData', JSON.stringify(tableData.value)); } catch (e) { void 0; }
...
<template>
  <div id="cuerpo">
    <div v-if="isMobileOrTablet" class="arrow-btn-container" style="display: none;">
